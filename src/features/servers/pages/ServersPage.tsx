...
export function ServersPage(): JSX.Element {
  ...
  const removeServer = useHexaStore((state) => state.removeServer);
  ...
  const onDelete = (serverId: string, serverName: string) => {
    const confirmed = window.confirm(`Confirmer la suppression de ${serverName} ?`);
    if (!confirmed) return;
  ...
  return (
    <section>
      ...
      <form className="inline-form" onSubmit={onSubmit}>
        ...
        <input value={notes} placeholder="Notes" onChange={(event) => setNotes(event.target.value)} />
          ...
          <tbody>
            {servers.map((server) => (
              <tr key={server.id}>
                ...
                <td>
                  <button type="button" onClick={() => onDelete(server.id, server.name)}>Supprimer</button>
                </td>
              </tr>
            ))}
          </tbody>
